...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'END:VCARD' 
prefix = ','
blocked = ["6285806408006@s.whatsapp.net"]   
limitawal = 100
memberlimit = 2
cr = '*BOT INI SUDAH TERVERIFIKASI*'

/******** OWNER NUMBER**********/
const ownerNumber = ["6289686610207@s.whatsapp.net","6281236547839@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SAYONARA... @${num.split('@')[0]}👋* \n_Hati-Hati Dijalan, semoga ga dipanggil sama Tuhan :D_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            break
                ...
                addKoinUser('6289686610207@s.whatsapp.net', fee)
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Bisa','Tidak Bisa','Coba Ulangi','Bisa banget']
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *P E S A N  B R O A D C A S T* ❯\n\n${body.slice(4)}`})
						}
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer pembuat bot kuh, jan di spam yah beb >_<',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
